...
class BaseModel(nn.Module):
    ...
    def fit_batch(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        validation_data=None, shuffle=True, callbacks=None, batch_str=None):
        """
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            try:
                with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                    for iter_index, (x_train, y_train) in t:
                        x = x_train.to(self.device).float()
                        y = y_train.to(self.device).float()
                        ...
                        if (iter_index % 100) == 0:
                            batch_str.append("batch:{0:<8d}".format(iter_index) +
                                             "current batch train loss:{0:8f}".format(total_loss.item() / batch_size) +
                                             "      " + str(self.evaluate(val_x, val_y)) + '\n')
            ...
            batch_str.append('Epoch {0}/{1}'.format(epoch + 1, epochs) + '\n' + eval_str + '\n\n')

        callbacks.on_train_end()

        today = datetime.today().strftime('%Y_%m_%d_%H_%m')
        with open('../data/taobao_data/new/batch_result_' + today + '.txt', 'w') as f:
            f.writelines(batch_str)
