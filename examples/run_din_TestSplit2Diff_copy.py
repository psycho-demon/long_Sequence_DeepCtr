...
def gen_dataset(user_df, item_df, item_cnt, feature_size, feature_columns, MAX_LEN_ITEM):
    ...
    for uid, hist in user_df:
        ...
        if test:
            ...
            test_true_length.append(len(item_part))
        else:
            cat_list = []
            item_list = []
            # btag_list = []
    ...
    test_behavior_length_idx = [cal_spilt_index(l) for l in test_true_length]
    test_behavior_length_idx = np.array(test_behavior_length_idx)
    test_x = []
    test_y = []
    for idx in range(max(test_behavior_length_idx) + 1):
        select_idx = test_behavior_length_idx == idx
        spilt_uid_array = test_uid_array[select_idx]
        spilt_iid_array = test_iid_array[select_idx]
        spilt_icate_array = test_icate_array[select_idx]
        spilt_label_array = test_label_array[select_idx]
        spilt_hist_iid_array = test_hist_iid_array[select_idx]
        spilt_hist_icate_array = test_hist_icate_array[select_idx]
        spilt_behavior_length = test_behavior_length[select_idx]
        spilt_feature_dict = {'user': spilt_uid_array, 'item': spilt_iid_array, 'item_cate': spilt_icate_array,
                             'hist_item': spilt_hist_iid_array, 'hist_item_cate': spilt_hist_icate_array,
                             "seq_length": spilt_behavior_length}
        spilt_test_x = {name: spilt_feature_dict[name] for name in get_feature_names(feature_columns)}
        spilt_test_y = spilt_label_array
        test_x.append(spilt_test_x)
        test_y.append(spilt_test_y)

    print("train and test sample completed")
    return x, y, test_x, test_y


def cal_spilt_index(length):
    idx = (length - 1) // 16
    if idx > 17:
        idx = 17
    return idx
...
def spilt_test_data(test_x, test_y):
    pass
...
if __name__ == "__main__":
    ...
    history = model.fit(x, y, batch_size=256, epochs=1, verbose=2)
    str_array = []
    for idx, (t_x, t_y) in enumerate(zip(test_x, test_y)):
        if idx + 1 < 18:
            s = "behavior length: {0:4} ~ {1:4}    ".format(idx * 16 + 1, idx * 16 + 16) \
                + str(model.evaluate(t_x, t_y)) + '\n'
        else:
            s = "behavior length: {0:4} ~         ".format(idx * 16 + 1) \
                + str(model.evaluate(t_x, t_y)) + '\n'
        str_array.append(s)

    with open('../data/taobao_data/new_200_test_split_dif_length.txt', 'w') as f:
