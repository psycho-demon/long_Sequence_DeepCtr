...
def analysis_1(user_df):
    user_df = list(user_df)
    user_length = []
    write_data = []
    for user_data in user_df:
        uid = user_data[0]
        u_length = len(user_data[1])
        user_length.append(u_length)
        u_str = str(uid) + '\t' + str(u_length) + '\n'
        write_data.append(u_str)

    max_len = max(user_length)
    print(f"max_len: {max_len}")
    max_len_id = (max_len - 1) // 16
    user_length_group = {x : 0 for x in range(0, max_len_id + 1)}
    for u_length in user_length:
        u_length_id = (u_length - 1) // 16
        user_length_group[u_length_id] += 1

    fig = plt.figure(figsize=(30, 8))
    X = list(user_length_group.keys())
    Y = list(user_length_group.values())
    X_label = [str(x*16) + '\n~\n' + str(x*16+16) for x in X]
    plt.xticks(X, X_label)  # 绘制x刻度标签
    plt.xlabel("UserBehavior_length")
    plt.ylabel("Number")
    plt.bar(X, Y)
    for a, b in zip(X, Y):
        plt.text(a, b, b)
    plt.savefig("../data/taobao_data/UserBehavior_length.png", dpi=400)
    with open('../data/taobao_data/UserBehavior_length.txt', 'w') as f:
        f.writelines(write_data)


def analysis_2(user_df):
    user_df = list(user_df)
    idx_count = 0
    write_data = []
    distance_data = []
    for user_data in user_df:
        uid = user_data[0]
        u_data = list(user_data[1]["iid"])
        target_item = u_data[-1]
        idx = u_data.index(target_item)
        distance = len(u_data) - 1 - idx
        distance_data.append(distance)
        if idx == len(u_data) - 1:
            idx = -1
            idx_count += 1
        write_data.append("uid:" + '{0:10}'.format(str(uid)) + "len:" + '{0:8}'.format(str(len(u_data)))
                          + "index:" + '{0:8}'.format(str(idx)) + "distance:" + '{0:8}'.format(str(distance)) + '\n')

    print(f"The number of target items at the end of the sequence : {idx_count}      ratio:{idx_count/len(user_df)}")
    # with open('../data/taobao_data/UserBehavior_target_index.txt', 'w') as f:
    #     f.writelines(write_data)
    fig = plt.figure(figsize=(20, 10))
    X = list(range(1, max(distance_data) + 1))
    Y = [distance_data.count(y) for y in range(1, max(distance_data) + 1)]
    # X_label = [str(x * 16) + '\n~\n' + str(x * 16 + 16) for x in X]
    # plt.xticks(X, X_label)  # 绘制x刻度标签
    plt.xlabel("Distance")
    plt.ylabel("Number")
    ...
    # for a, b in zip(X, Y):
    #     plt.text(a, b, b)
    plt.savefig("../data/taobao_data/Distance.png", dpi=400)
...
if __name__ == "__main__":
    ...
    #               metrics=['auc', 'logloss'])
    # history = model.fit(x, y, batch_size=256, epochs=1, verbose=2, validation_data=(test_x, test_y))
