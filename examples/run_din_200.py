...
if __name__ == "__main__":
    ...
    model.compile('adam', 'binary_crossentropy',
                  metrics=['auc', 'logloss'])
    history = model.fit(x, y, batch_size=batch_size, epochs=1, verbose=2, validation_data=(test_x, test_y))
    print(batch_size)

    # batch_str = []
    # batch_str.append("NEW   max_len: " + str(MAX_LEN_ITEM) + " batch_size: " + str(batch_size) + '\n')
    # history = model.fit_batch(x, y, batch_size=batch_size, epochs=3, verbose=2, validation_data=(test_x, test_y), batch_str=batch_str)
